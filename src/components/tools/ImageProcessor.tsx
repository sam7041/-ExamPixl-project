...
import { Upload, FileMinus, Maximize, Minimize2, Download, RefreshCw, CheckCircle2, AlertCircle, X, Image as ImageIcon, Wand2, Type, Crop, RotateCw, FlipHorizontal, Square, Circle, Palette, Sparkles, Settings, Plus, Trash2, FileText, Shield, Sliders, Settings2, Zap, Info, ImagePlus, Monitor } from 'lucide-react';
...
const EXTRA_IMAGE_SEO: Record<string, { title: string; description: string }> = {
  'WebP Optimizer': { title: 'Convert Image to WebP Free Online', description: 'Convert JPG and PNG images to optimized WebP files online for free. Improve loading speed while keeping good quality.' },
  'Strip Metadata': { title: 'Remove EXIF Metadata from Image Free', description: 'Strip EXIF and hidden metadata from JPG, PNG, and WebP images online for better privacy and smaller files.' },
  'Progressive JPEG': { title: 'Create Progressive JPEG Online Free', description: 'Convert regular JPG images into progressive JPEGs online for smoother loading and better perceived performance.' },
  'Round Corners': { title: 'Round Image Corners Free Online', description: 'Add rounded corners to images online for free. Adjust corner radius and export clean JPG, PNG, or WebP files.' },
  'Borders & Padding': { title: 'Add Border and Padding to Image Free', description: 'Add borders, spacing, and canvas padding to your images online for free. Great for forms, posters, and social media.' },
  'Change Background': { title: 'Change Image Background Color Free', description: 'Replace the background of your image with a custom color online for free. Great for white background form photos.' },
  'Collage Maker': { title: 'Photo Collage Maker Free Online', description: 'Create image collages and grid layouts online for free. Combine multiple photos into a single clean composition.' },
  'Merge Images': { title: 'Merge Images Free Online', description: 'Combine multiple images into a single file online for free. Great for photo strips, documents, and quick visual merges.' },
  'Responsive Gen': { title: 'Generate Responsive Image Sizes Free', description: 'Generate multiple responsive image sizes online for free for web, mobile, and modern frontend image workflows.' },
  'White Background': { title: 'Make Image Background White Free Online', description: 'Remove the existing background and replace it with a clean white background online for free.' },
  'Triple Signature': { title: 'Triple Signature Generator Free Online', description: 'Create a three-signature stacked image online for UPSC and similar exam form requirements.' },
  'Declaration Generator': { title: 'Exam Declaration Image Generator Free', description: 'Create declaration images for banking and exam forms online with clean formatting and quick export.' },
  'Signature Extractor': { title: 'Extract Signature from Image Free', description: 'Extract a clean signature from an image online for free by removing unwanted background and noise.' },
  'OCR Scanner': { title: 'Extract Text from Image Free Online', description: 'Use OCR to extract text from image files online for free. Great for forms, scanned images, and screenshots.' },
};

const IMAGE_PATHS: Record<string, string> = {
  'Compress Image': '/image/compress',
  'Format Converter': '/image/convert',
  'WebP Optimizer': '/image/webp',
  'Strip Metadata': '/image/strip-exif',
  'Progressive JPEG': '/image/progressive',
  'Crop Image': '/image/crop',
  'Rotate & Flip': '/image/transform',
  'Round Corners': '/image/round',
  'Borders & Padding': '/image/border',
  'Remove Background': '/image/remove-bg',
  'Change Background': '/image/bg-color',
  'Filters & Effects': '/image/filters',
  'Adjustments': '/image/adjust',
  'Watermark': '/image/watermark',
  'Batch Process': '/image/bulk',
  'Image to PDF': '/image/to-pdf',
  'Collage Maker': '/image/collage',
  'Merge Images': '/image/merge',
  'Responsive Gen': '/image/responsive',
  'White Background': '/image/white-background',
  'Triple Signature': '/image/triple-signature',
  'Declaration Generator': '/image/declaration',
  'Signature Extractor': '/image/signature-extractor',
  'OCR Scanner': '/image/ocr',
};


export default function ImageProcessor({ title, description }: ImageProcessorProps) {
  const seo = EXTRA_IMAGE_SEO[title] ?? IMAGE_SEO[title] ?? { title, description };
  const canonicalPath = IMAGE_PATHS[title];
  useSEO({
    title: seo.title,
    description: seo.description,
    canonical: canonicalPath ? `https://exampixl.vercel.app${canonicalPath}` : undefined,
  });
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          {files.length === 0 ? (
            <div
              ...
              onDrop={onDrop}
              className="glass-card relative overflow-hidden border-dashed border-2 border-slate-300 bg-slate-50/50 flex flex-col items-center justify-center text-center hover:border-brand-400 hover:bg-brand-50/30 transition-all min-h-[420px] sm:min-h-[500px] p-6 sm:p-10 lg:p-16 group"
            >
              <input
                type="file"
                ref={fileInputRef}
              ...
              <div className="relative z-10 flex flex-col items-center">
                <div 
                  ...
                  className="h-24 w-24 sm:h-28 sm:w-28 bg-white rounded-[2rem] shadow-2xl shadow-brand-500/10 flex items-center justify-center text-brand-600 mb-6 sm:mb-8 border border-slate-100 cursor-pointer hover:scale-110 transition-transform duration-500"
                >
                ...
                <p className="text-slate-500 mb-8 sm:mb-12 max-w-md text-base sm:text-lg leading-relaxed">
                ...
                <div className="w-full max-w-sm">
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="w-full flex flex-col items-center gap-3 p-5 sm:p-6 rounded-3xl bg-white border border-slate-200 hover:border-brand-500 hover:bg-brand-50/50 transition-all shadow-sm group/btn"
                  >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover/btn:text-brand-600">Upload Images</span>
                  </button>
                </div>
              </div>
                ...
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    {processedFiles.length > 1 && !isImageToPDF && (
                      <button 
                        onClick={downloadZip}
                        className="flex items-center justify-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-2xl text-sm font-bold hover:bg-slate-800 shadow-xl transition-all w-full sm:w-auto"
                      >
                        <Download size={18} />
                        Download All (ZIP)
                      </button>
        ...
        <div className="space-y-6">
          <div className="glass-card p-5 sm:p-6 lg:p-8 bg-white lg:sticky lg:top-24 shadow-xl shadow-slate-200/50">
